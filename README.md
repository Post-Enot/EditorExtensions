...
## Атрибуты поиска ссылок на компоненты

В некоторых случаях бывает полезным иметь возможность автоматически получать ссылки на компоненты, например, при добавлении компонента на объект. Пакет предоставляет для этого механизм инициализации полей, а также атрибуты маркировки инициализируемых полей.

**Предоставляемые атрибуты:**

- `GetSelfAttribute`. Указывает, что ссылку на компонент следует искать на том же объекте.
- `GetInChildrenAttribute`. Указывает, что ссылку на компонент следует искать на том же объекте и дочерних объектах.
- `GetInChildrenOnlyAttribute`. Указывает, что ссылку на компонент следует искать только на дочерних объектах.
- `GetInParentAttribute`. Указывает, что ссылку на компонент следует искать на том же объекте и на родительских объектах.
- `GetInParentOnlyAttribute`. Указывает, что ссылку на компонент следует искать только на родительских объектах.
- `GetInHierarchyAttribute`. Указывает, что ссылку на компонент следует искать на том же объекте, на дочерних объектах и на родительских объектах.
- `GetInHierarchyOnlyAttribute`. Указывает, что ссылку на компонент следует искать на дочерних объектах и на родительских объектах.

Сами атрибуты не реализуют какую-либо логику, а лишь указывают механизму инициализации, какие поля и как необходимо инициализировать. Для непосредственной инициализации необходимо вызвать метод `PostEnot.Toolkits.RefUtility.InitReferences(MonoBehaviour monoBehaviour)`. Благодаря подобному разделению вы вольны сами определять, в какой момент и при каких обсоятельствах необходимо инициализировать ссылку.

Так, например, вы можете поместить вызов метода в сообщение Unity `Reset`, благодаря чему инициализация будет происходить при добавлении компонента на объект или вызову действия `Reset` из контекстного меню:

```csharp
using UnityEngine;
using PostEnot.Toolkits;

public sealed class InitSample : MonoBehaviour
{
    [SerializeField, GetSelf] private Animator animator;

    private void Reset() => RefUtility.InitReferences(this);
}
```

Метод также корректно обрабатывает массивы и списки: вы можете использовать те же атрибуты, чтобы получить ссылки на все компоненты, что удовлетворяют критериям поиска:
...
public sealed class InitSample : MonoBehaviour
{
    [SerializeField, GetInChildrenOnly] private Transform[] allChildren;
    [SerializeField, GetInParentOnly] private List<Transform> allParents;

    private void Reset() => RefUtility.InitReferences(this);
}
```

Механизм инициализации с помощью `RefUtility.InitReferences` предназначен для использования только в редакторе: вызов данного метода в Runtime хотя и возможен, но не приведёт ни к каким последствиям.
